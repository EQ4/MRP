...
//  touchkeys
//
//  Created by Andrew McPherson on 31/01/2013.
...
class IIRFilter : public Node<DataType> {
public:
	...
	IIRFilter(capacity_type capacity, Node<DataType>& input) : Node<DataType>(capacity), input_(input),
      autoCalculate_(false), inputHistory_(0), outputHistory_(0), lastInputIndex_(0) {
	}
    
	// Copy constructor
	IIRFilter(IIRFilter<DataType> const& obj) : Node<DataType>(obj), input_(obj.input_), autoCalculate_(obj.autoCalculate_),
    ...
    typename Node<DataType>::return_value_type calculate(int maximumLookback = -1) {
        ...
        if(maximumLookback >= 0 && index < input_.endIndex() - 1 - maximumLookback) {
            std::cout << "IIRFilter: clearing history at index " << index << std::endl;
